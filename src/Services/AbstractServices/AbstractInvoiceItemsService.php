...
class AbstractInvoiceItemsService
{
    public static function get(?InvoiceItemsQueryFilter $filter = null, array $params = []) : Collection|LengthAwarePaginator
    {
        ...
        */
        if($filter == null) {
            $filter = new InvoiceItemsQueryFilter($request);
        }
    ...
    {
        $object = InvoiceItems::where('uuid', $objectId)->first();

        if(!$object) {
            throw new ModelNotFoundException('Cannot find the InvoiceItems object with the id: ' . $objectId);
        }

        $availableAction = AvailableActions::where('name', $action)
            ->where('input', 'NextDeveloper\Accounting\InvoiceItems')
            ->first();

        if(!$availableAction) {
            throw new NotFoundException('Cannot find the action "' . $action . '" for NextDeveloper\Accounting\InvoiceItems.');
        }
