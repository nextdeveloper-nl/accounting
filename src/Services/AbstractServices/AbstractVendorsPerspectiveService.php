...
/**
 * This class is responsible from managing the data for VendorsPerspective
 *
 * Class VendorsPerspectiveService.
 *
 * @package NextDeveloper\Accounting\Database\Models
 */
class AbstractVendorsPerspectiveService
{
    public static function get(?VendorsPerspectiveQueryFilter $filter = null, array $params = []) : Collection|LengthAwarePaginator
    {
        ...
        */
        if($filter == null) {
            $filter = new VendorsPerspectiveQueryFilter($request);
        }
        ...
        $model = VendorsPerspective::filter($filter);
     ...
     * @return mixed
     */
    public static function getByRef($ref) : ?VendorsPerspective
    {
        return VendorsPerspective::findByRef($ref);
    }

    public static function getActions()
    {
        $model = VendorsPerspective::class;
    ...
    {
        $object = VendorsPerspective::where('uuid', $objectId)->first();

        if(!$object) {
            throw new ModelNotFoundException('Cannot find the VendorsPerspective object with the id: ' . $objectId);
        }

        $availableAction = AvailableActions::where('name', $action)
            ->where('input', 'NextDeveloper\Accounting\VendorsPerspective')
            ->first();

        if(!$availableAction) {
            throw new NotFoundException('Cannot find the action "' . $action . '" for NextDeveloper\Accounting\VendorsPerspective.');
        }
     ...
     *
     * @param  $id
     * @return VendorsPerspective|null
     */
    public static function getById($id) : ?VendorsPerspective
    {
        return VendorsPerspective::where('id', $id)->first();
    }
    ...
    {
        try {
            $obj = VendorsPerspective::where('uuid', $uuid)->first();
    ...
    public static function create(array $data)
    {
        ...
        try {
            $model = VendorsPerspective::create($data);
        } catch(\Exception $e) {
            throw $e;
        }
     ...
     *
     * @param  array $data
     * @return VendorsPerspective
     */
    public static function updateRaw(array $data) : ?VendorsPerspective
    {
    ...
    {
        $model = VendorsPerspective::where('uuid', $id)->first();
