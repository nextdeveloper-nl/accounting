 ...
 */
class AbstractPaymentCheckoutSessionsService
{
    public static function get(?PaymentCheckoutSessionsQueryFilter $filter = null, array $params = []) : Collection|LengthAwarePaginator
    {
        ...
        */
        if($filter == null) {
            $filter = new PaymentCheckoutSessionsQueryFilter($request);
        }
        ...
        $model = PaymentCheckoutSessions::filter($filter);
    ...
    {
        return PaymentCheckoutSessions::all();
    }
     ...
     * @return mixed
     */
    public static function getByRef($ref) : ?PaymentCheckoutSessions
    {
        return PaymentCheckoutSessions::findByRef($ref);
    }

    public static function getActions()
    {
        $model = PaymentCheckoutSessions::class;
    ...
    {
        $object = PaymentCheckoutSessions::where('uuid', $objectId)->first();

        if(!$object) {
            throw new ModelNotFoundException('Cannot find the PaymentCheckoutSessions object with the id: ' . $objectId);
        }

        $availableAction = AvailableActions::where('name', $action)
            ->where('input', 'NextDeveloper\Accounting\PaymentCheckoutSessions')
            ->first();

        if(!$availableAction) {
            throw new NotFoundException('Cannot find the action "' . $action . '" for NextDeveloper\Accounting\PaymentCheckoutSessions.');
        }
     ...
     *
     * @param  $id
     * @return PaymentCheckoutSessions|null
     */
    public static function getById($id) : ?PaymentCheckoutSessions
    {
        return PaymentCheckoutSessions::where('id', $id)->first();
    }
    ...
    {
        try {
            $obj = PaymentCheckoutSessions::where('uuid', $uuid)->first();
     ...
     */
    public static function updateRaw(array $data) : ?PaymentCheckoutSessions
    {
    ...
    {
        $model = PaymentCheckoutSessions::where('uuid', $id)->first();
