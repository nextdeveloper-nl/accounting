...
/**
 * This class is responsible from managing the data for IntegratorsPerspective
 *
 * Class IntegratorsPerspectiveService.
 *
 * @package NextDeveloper\Accounting\Database\Models
 */
class AbstractIntegratorsPerspectiveService
{
    public static function get(?IntegratorsPerspectiveQueryFilter $filter = null, array $params = []) : Collection|LengthAwarePaginator
    {
        ...
        */
        if($filter == null) {
            $filter = new IntegratorsPerspectiveQueryFilter($request);
        }
        ...
        $model = IntegratorsPerspective::filter($filter);
    ...
    {
        return IntegratorsPerspective::all();
    }
     ...
     * @return mixed
     */
    public static function getByRef($ref) : ?IntegratorsPerspective
    {
        return IntegratorsPerspective::findByRef($ref);
    }

    public static function getActions()
    {
        $model = IntegratorsPerspective::class;
    ...
    {
        $object = IntegratorsPerspective::where('uuid', $objectId)->first();

        if(!$object) {
            throw new ModelNotFoundException('Cannot find the IntegratorsPerspective object with the id: ' . $objectId);
        }

        $availableAction = AvailableActions::where('name', $action)
            ->where('input', 'NextDeveloper\Accounting\IntegratorsPerspective')
            ->first();

        if(!$availableAction) {
            throw new NotFoundException('Cannot find the action "' . $action . '" for NextDeveloper\Accounting\IntegratorsPerspective.');
        }
     ...
     *
     * @param  $id
     * @return IntegratorsPerspective|null
     */
    public static function getById($id) : ?IntegratorsPerspective
    {
        return IntegratorsPerspective::where('id', $id)->first();
    }
    ...
    {
        try {
            $obj = IntegratorsPerspective::where('uuid', $uuid)->first();
    ...
    public static function create(array $data)
    {
        ...
        try {
            $model = IntegratorsPerspective::create($data);
        } catch(\Exception $e) {
            throw $e;
        }
     ...
     *
     * @param  array $data
     * @return IntegratorsPerspective
     */
    public static function updateRaw(array $data) : ?IntegratorsPerspective
    {
    ...
    {
        $model = IntegratorsPerspective::where('uuid', $id)->first();
