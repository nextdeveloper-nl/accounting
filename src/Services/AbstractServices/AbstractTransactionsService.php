...
class AbstractTransactionsService
{
    public static function get(?TransactionsQueryFilter $filter = null, array $params = []) : Collection|LengthAwarePaginator
    {
        ...
        */
        if($filter == null) {
            $filter = new TransactionsQueryFilter($request);
        }
    ...
    public static function doAction($objectId, $action, ...$params)
    {
        ...
        if(!$object) {
            throw new ModelNotFoundException('Cannot find the Transactions object with the id: ' . $objectId);
        }
        ...
        if(!$availableAction) {
            throw new NotFoundException('Cannot find the action "' . $action . '" for NextDeveloper\Accounting\Transactions.');
        }
